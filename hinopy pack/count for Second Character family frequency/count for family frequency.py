# -*- coding: utf-8 -*-
print('2字熟語の二番目の文字のファミリーフリークエンシーを計算するプログラムです。')
...
rawdata = f.readlines()
f.close()
#二番目の文字のみのIndexリストを作成
Index = [x.strip().split()[0] for x in rawdata]
#二番目の文字が対応する2字熟語（その熟語の発音を除去）のリストを作成（中身は複数のリスト）
...
#結果をファイルに書き込む部分です。
f = open('Second_Character_family_frequency.dat', 'w', encoding = 'utf-8')
